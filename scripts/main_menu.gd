...
func _on_quit_pressed():
	...
	if PlayerData.is_logged_in:
		# Save settings FIRST — before logout() clears save_file_data
		PlayerData.save_settings()
		print("Settings saved: difficulty=%s guided=%s" % [
			Settings.difficulty_level,
			Settings.guided_mode_enabled
		])
		# Logout AFTER the file has been written
		PlayerData.logout()
